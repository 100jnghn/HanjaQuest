...
namespace BeyondLimitsStudios
{
    ...
    {
        public class BoardScript : MonoBehaviour, DrawingBoard
        {
            [SerializeField]
            ...
            public static Action SetTex;
            public GameObject hanjaSkillObject;
            ...
            public void SaveAndAssignTexture()
            {
                Texture2D texture = GetTexture();
                Debug.Log("Tex func called");

                if (texture != null)
                {

                    string fileName = "drawnImage";
                    string filePath = Path.Combine(Application.dataPath, "DrawnTextures", fileName + ".png");
                    File.WriteAllBytes(filePath, texture.EncodeToPNG());
                    AssetDatabase.Refresh();

                    Debug.Log("Tex !=null");

                    Text textComponent = hanjaSkillObject.GetComponent<Text>();
                    if (textComponent != null)
                    {
                        Sprite sprite = Sprite.Create(texture, new Rect(0, 0, texture.width, texture.height), new Vector2(0.5f, 0.5f));
                        textComponent.material.mainTexture = sprite.texture;
                    }
